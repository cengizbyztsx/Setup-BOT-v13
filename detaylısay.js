...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    const cengiz = message.channel.send({
      embeds: [embed
      ...
      <t:${Math.floor(Date.now() / 1000)}:R> **Tarihli Sunucu Verisi**
       ...
       Sunucumuza **1 saat** içerisinde ${rakam(message.guild.members.cache.filter(cengiz => (new Date().getTime() - cengiz.joinedTimestamp) < 1000 * 60 * 60).size)} kullanıcı giriş yapmış.
       **1 gün** içerisinde ${rakam(message.guild.members.cache.filter(cengiz => (new Date().getTime() - cengiz.joinedTimestamp) < 1000 * 60 * 60 * 24).size)} kullanıcı giriş yapmış.
       Sunucumuza **1 hafta** içerisinde ${rakam(message.guild.members.cache.filter(cengiz => (new Date().getTime() - cengiz.joinedTimestamp) < 1000 * 60 * 60 * 24 * 7).size)} kullanıcı giriş yapmış.
       Sunucumuza **1 ay** içerisinde ${rakam(message.guild.members.cache.filter(cengiz => (new Date().getTime() - cengiz.joinedTimestamp) < 1000 * 60 * 60 * 24 * 30).size)} kullanıcı giriş yapmış.
