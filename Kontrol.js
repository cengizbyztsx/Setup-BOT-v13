...
module.exports = {
    ...
    let cengizcim = message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0)
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ embeds: [cengiz], components: [row]})
    ...
    let cengiz = message.guild.members.cache.filter(member => !member.roles.cache.has(etkinlik) || !member.roles.cache.has(cekilis))
    button.reply({ content:`
Etkinlik/Çekiliş rolü olmayan ${cengiz.size} kullanıcıya etkinlik, çekiliş rolleri verildi !`})
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`})
    ...
    button.reply({ content:`
Kayıtsız rolü olmayan ${cengizcim.size} kullanıcıya kayıtsız rolü verildi !

Kayıtsız Rolü verilen kullanıcılar;
${cengiz.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `})
